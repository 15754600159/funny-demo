define(function(require){
  ...
  var nav= require('core/app/fixednav');
    ...
    var getTemplateHtml = function (type, obj) {
        switch(type){
            case 'dynamicList':
                ...
                if(obj.totalCount == 0){
                    datalist.push(['<tr data-dynamic-id=""><td colspan="6" class="isEmpty">暂无数据</td></tr>'].join());
                }else{
                    for(i;i<data.length;i++){
                        ...
                        datalist.push(['<tr ids="' + memberIds.join(',') + '" data-dynamic-id="' + itemData.gangId + '">',
    ...
    var filterDataAction = function(){
        ...
        $('.forewarn-box select',$page).select2({
    ...
    var tableAction = function(){
            ...
            callbacks:{
                onTotalScroll: function(){
                    ...
                    if(isFilter){
                        ...
                        getData('dynamicList',orderParams);
                    }else{
                        getData('dynamicList',[]);
                    }
                }
        ...
        $('.forewarn-box .forewarn-list .data-list table tbody tr',$page).off('click').on('click',function(){
            if($(this).attr('data-dynamic-id').length == 0){
                $(docParameters['dynamicDetails'].el,$page).html('');
                return false;
            }
            $(this).addClass('active').siblings().removeClass('active');
            var params = [];
            params.push('groupId=' + $(this).attr('data-dynamic-id'));
            getData('dynamicDetails',params);
        ...
        // if($('.forewarn-box .forewarn-list .data-list table tbody tr',$page).length > 0 && $(docParameters['dynamicDetails'].el + '>div',$page).length == 0){
        if(isClicked){
            isClicked = false;
