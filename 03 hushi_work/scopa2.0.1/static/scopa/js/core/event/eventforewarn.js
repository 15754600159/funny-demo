define(function(require){
    ...
    var getTemplateHtml = function (type, obj) {
        switch(type){
            case 'qbyjList':
                ...
                if(data.length == 0){
                    datalist.push(['<tr data-qbyj-id=""><td colspan="9" class="isEmpty">暂无数据</td></tr>'].join());
                }else{
                    for(i;i<data.length;i++){
                        var itemData = data[i],
                            j = 0,
                            arr = [];
                        datalist.push(['<tr data-qbyj-id="' + itemData.id + '">',
    ...
    var init = function(categoryId){
        ...
        parameters = '?applicantId=' + userId;
        $mainBox = $('.list-container .eventforewarn',$page);
        categoryIds = [];
        initial = true;
        if(categoryId){
    ...
    var filterDataAction = function(){
        ...
        $('.filters .label-filter',$mainBox).off('click').on('click',function(){
            var that = this;
            labelTree.init('eventforewarn',categoryIds,function(data){
                var len = data.length,
                    params = [];
                categoryIds = [];
                pageNo = 1;
                ...
                filterData();
                // getData('qbyjList',params);
            });
            mining.utils.serverLog(419);
        });

        //类型
        $('.filters .qbyj-types select',$mainBox).off('change').on('change',function(){
            initial = true;
            pageNo = 1;
            filterData();
            var typesLog = $('.filters .qbyj-types select',$mainBox).val();
            mining.utils.serverLog(421,typesLog);
        });
        ...
        $('.filters .searchbox .searchbox-btn',$mainBox).off('click').on('click',function(){
            ...
            mining.utils.serverLog(420,keywordsLog);
        });
    ...
    var filterData = function(){
        var params = [],
            ...
            types = $filter.find('.qbyj-types select').val(),
    ...
    var resetValue = function(isResetSelect){
        ...
        if(isResetSelect){
            $('.filters .qbyj-types select',$mainBox).select2('val','default');
