                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if(data[i].location||data[i].location.indexOf(',')){
                            ...
                            if(newstring === oldstring){
                                console.info(data[i]);
                                var strHTML1 = '<p>车牌号:</br>'+(data[i].plateNo==undefined?'':data[i].plateNo)+'</p>'+'<p>所有人:</br>'+data[i].name+'</p><p>来源:</br>'+data[i].source+'</p><p> 车牌号类型:</br>'+data[i].plateType+'<p>警员:</br>'+data[i].policename+'<p>';
	        ...
	        if(data.msg.content && data.msg.content.length > 0 ) {
	            ...
	            $.each(data.msg.content, function (index, item) {
	               ...
	               $("#tableBody").append(
	                    ...
	                    "<td class='aa lineh'  tid='"+(item.plateNo==undefined?'':item.plateNo)+"'><img title='查看' class='cz see' data='"+JSON.stringify(item)+"' src='../img/ck.png'><img title='地图' idcardNo='"+(item.plateNo==undefined?'':item.plateNo)+"'  class='cz fuck lineh' src='../img/dt.png'></td>"+
                        "</tr>"
	                );
	            });
	        }
