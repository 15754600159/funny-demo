...
function content(data) {
	...
	$.each(data, function(k, v) {
		...
		if (v.finish == 0) {
			...
			statusStr = "执行中";
			statusHtml = '<div class="progress-bar" role="progressbar" style="width: 0%;height:20px;" id="progressbar-'+v.id+'">&nbsp;</div>';
			btnHtml = '<div id="task-btn-'+v.id+'" style="display:none;"><a href="#" onclick="DataDetail('+v.id+')">详情</a>'+ 
			'<a href="#" onclick="DelData(' + v.id + ',this)">删除</a></div>';
			// 记录当前列表状态
			taskIdMap[v.id] = false;
		...
		$('#task_tbody').append(
			...
			'<td>' + statusHtml +'</td>' +
			'<td class="cz taskDetail1">'+ btnHtml +'&nbsp;</td>' +
			'</tr>'
		)
	});
	
	for(var id in taskIdMap){
		jinDu(id);
	}
}
	...
	function run() {
		...
		if(finish) {
			...
			if(i >= 100) {
				...
				$.ajax({
					type:"get",
					url:bjhchost+ "/task/info/view/"+id+'/'+getUser().loginName,
					async:true,
					success: function (data) {
						console.log(data);
						if(data.finish == 1){
