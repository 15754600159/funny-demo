...
function Daochu() {
	...
	$('#task_daochu').on('click', function() {
		var tags = getCheckTags();
		var sfzh = getIdcards();
		// console.log(bjhchost+ "/task/export/search?jobId=" + id + "&userId="+sessionId+ "&idCard="+sfzh+"&tags="+tags);
		location.href = bjhchost+ "/task/export/search?jobId=" + id + "&userId="+sessionId+ "&idCard="+sfzh+"&tags="+tags;
	})
}
//搜索
function SearchTask() {
...
function search(){
	var id = GetQueryString('taskId');
	var str = getCheckTags();
	var sfzh = getIdcards;

	$.ajax({
		type: "post",
		url: bjhchost+ "/task/detail/search",
		data: {
			jobId: id,
			tags: str,
			idCard: sfzh
	...
	if(sfzhstr) {
		...
		for (var i = 0; i < sfzhArry.length; i++) {
			if (!reg.test(sfzhArry[i]) || reg1.test(sfzhArry[i])) {
				...
				// 去重
				if(!sfzhMap[sfzhArry[i]]){
					sfzh += sfzhArry[i] + ",";
				}
				sfzhMap[sfzhArry[i]] = true;
			}
		}
	}
	return sfzh;
