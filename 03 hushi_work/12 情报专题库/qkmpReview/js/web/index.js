...
function initData(){
	var sfzh = GetQueryString('idCard');
	$('#task_text').val(sfzh);
...
function MoBan() {
	$('#moban').on('click',function () {
		...
		// location.href = 'http://10.101.139.21:8777/xls/model.xls';
	})
}
//弹框1
function dialog() {
	...
	$('.ok').on('click', function() {
		...
		$('#liulan-form').ajaxSubmit({
			...
			success: function(data) {
				...
				if(total == 0){
					alert('上传文件为空,请重新选择文件上传。');
					$('.ok').attr('disabled',false);
					return false;
				}
				if(total > 5000){
					alert('不能上传超过5000条的数据,请修改文件重新上传!');
					$('.ok').attr('disabled',false);
					return false;
				}
