...
gulp.task('release:copy', function(cb) {
	gulp.src(['favicon.ico', 'fonts/**', 'images/**', 'pages/**', 'index.html', 'css/*.min*.css', 'js/*.min*.js', 'template/**'], {
		cwd: paths.dist.root,
		base: paths.dist.root
	}).pipe(gulp.dest(paths.release.root + 'rytzfx-web/')).on('end', cb);
});

gulp.task('release:compress', function(cb) {
	gulp.src(paths.release.root + 'rytzfx-web/**').pipe(zip('rytzfx-web.zip')).pipe(gulp.dest(paths.release.root)).on(
		'end', cb);
});
