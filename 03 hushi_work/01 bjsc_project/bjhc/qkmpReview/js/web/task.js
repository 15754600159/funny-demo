			...
			success: function (data) {
				...
				$.each(data, function(k,v) {
					...
					if (v.finish == 0) {
						...
						loadBtn = '<a href="#" onclick="Load(\''+v.id+'\',\''+v.seq+'\','+v.finish+',this);">执行</a>';
					}if (v.finish == 1) {
						statusStr = "执行完成";
					...
					$('#task_tbody').append(
						...
						'<td>' + statusHtml +'</td>' +
						'<td class="cz taskDetail1"><a href="#" onclick="DataDetail('+v.id+')">详情</a>'+ 
						'<a href="#" onclick="DelData(' + v.id + ',this)">删除</a>'+ loadBtn +
						'</td>' +
						'</tr>'
					)
				});
