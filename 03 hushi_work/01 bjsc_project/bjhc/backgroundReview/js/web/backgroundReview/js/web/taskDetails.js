...
function Daochu() {
	...
	$('#task_daochu').on('click', function() {
		/*$.ajax({
			type: "get",
			url: "http://10.101.139.21:8777/bjhc/task/export/" + id + '/' + sessionId,
			async: true,
			success: function(data) {
				console.log(data)
			}
		});*/
		location.href = "http://10.101.139.21:8777/bjhc/task/export/" + id + '/' + sessionId
	})
}
//搜索
function SearchTask() {
	$('#task_btn').on('click', function() {
		...
		$.ajax({
			type: "get",
			url: "http://10.101.139.21:8777/bjhc/task/detail/detail/" + str + '/' + jobId,
			async: true,
			success: function(data) {
				...
				$.each(data1, function(k, v) {
					...
					$('.task-detail-box').append(
						...
						'<div class="img-box">' +
						'<img src="http://yp.hsga.nn/sjz/gongan/alarm/photoInfo?SFZH=' + v.idCard + '" />' +
						'</div>' +
						...
						'<button type="button" class="btn btn-default" onclick="toSCOPA(\'' + data1.idCard + '\')">跳到图析</button>' +
						'<button type="button" class="btn btn-default" style="display:none">人工研判</button>' +
						'</div>' +
						'</div>'
					);
				});
...
function getData() {
	...
	if(!reg.test(sfzh) || reg1.test(sfzh)) {
		...
		$.ajax({
			type: "get",
			url: "http://10.101.139.21:8777/bjhc/task/info/one/" + newsfzh,
			async: true,
			success: function(data) {
...
function getTaskData() {
	...
	$.ajax({
		type: "get",
		url: "http://10.101.139.21:8777/bjhc/task/detail/get/" + id,
		async: true,
		success: function(data) {
...
function SxData() {
	$('#ck_btn').on('click', function() {
		...
		$.ajax({
			type: "post",
			url: "http://10.101.139.21:8777/bjhc/task/detail/tag",
			data: {
				id: id,
				tags: str
			},
