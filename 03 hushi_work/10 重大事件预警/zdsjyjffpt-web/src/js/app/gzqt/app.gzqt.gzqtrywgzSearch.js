app.gzqt.gzqtrywgzSearch = {
	...
	refresh: function() {
		app.gzqt.gzqtrywgzSearch.reset();
		app.gzqt.gzqtrywgzSearch.initSearchForm(0, 10, app.gzqt.gzqtrywgzSearch.pagination);
	},
			...
			success: function(result) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					$("#qtrywgzgl_table").append(
						...
						"<td>" + data[i].flag + "</td>" +
						"<td>" +
						'<button class="btn btn-sm btn-primary" type="submit" onclick="app.gzqt.toqtrywgzViewPage(\'' + data[i].id +
						'\')">查看</button>' +
						'<button class="btn btn-sm btn-primary" type="submit" style="margin-left:10px;" onclick="app.gzqt.toqtrywgzUpdatePage(\'' +
						data[i].id + '\')">修改</button>' +
						'<button class="btn btn-sm btn-primary" style="margin-left:10px;" onclick="app.gzqt.gzqtrywgzdel.delone(\'' +
						data[i].id + '\');" >删除</button>' +
						"</td>" +
						"</tr >"
					);
