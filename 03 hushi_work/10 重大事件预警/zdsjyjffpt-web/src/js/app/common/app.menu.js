	...
	initSidebarMenu: function() {
		...
		$(document).ready(function() {
			...
			$("nav > ul > li").on('click', function() {
				$(this).siblings('li').find('.active').removeClass('active');
			...
			$('.navbar').on('click', '.nav > li', function() {
		...
		$('#sidebar-menu').on('click', '.sidebar-menu-item', function() {
			app.ajaxCount = 0; // 切换页面，ajax计数器清零
			var $menu = $(this);
		...
		$('.sencend-menu').on('click', '.sidebar-menu-item', function() {
			app.ajaxCount = 0; // 切换页面，ajax计数器清零
			var $menu = $(this);
			$menu.parent().addClass('active').siblings('.active').removeClass('active');
			var page = $menu.data('page');
			console.log(page);
			if (page) {
				app.nav.toPage(page);
