app.zdsjyjmxgl.add = {
	...
	init: function() {
		...
		app.time.init();
		app.zdsjyjmxgl.add.checks();
		app.zdsjyjmxgl.add.mgsq();
		app.zdsjyjmxgl.add.select();
		this.initForm();
		this.mxpzbdfz();
		//app.zdsjyjmxgl.initSearchForm(0, 1, app.zdsjyjmxgl.pagination);
		// 弹框 'x'或者'取消' 点击关闭弹窗
		$('.btn-window-close').off('click').on('click', function() {
			$('.window').addClass('popOut'); //增加关闭效果
			app.zdsjyjmxgl.add.close()
		});
		// 保存按钮
		$('footer .btn_submit').on('click', function() {
			app.zdsjyjmxgl.add.updata()
		});
	},
	mgsq: function() {
		// $("#wggzqt-caijishijianstart").val(app.time.last());
	...
	close: function() {
		$m.page.closePage(app.zdsjyjmxgl.add.url, 'fade', 'container-wrapper');
	},
	del: function() {
	...
	showMenu: function(id) {
		app.zdsjyjmxgl.add.inputId = id;
		console.info(app.zdsjyjmxgl.add.inputId);
		var cityObj = $("#" + app.zdsjyjmxgl.add.inputId);
		var cityOffset = $("#" + app.zdsjyjmxgl.add.inputId).offset();
		$("#" + app.zdsjyjmxgl.add.inputId + "Content").css({
			left: cityOffset.left + "px",
			top: cityOffset.top + cityObj.outerHeight() + 2 + "px"
		}).slideDown("fast");
		$("body").bind("mousedown", app.zdsjyjmxgl.add.onBodyDown);
	},
	hideMenu: function() {
		$("#" + app.zdsjyjmxgl.add.inputId + "Content").fadeOut("fast");
		$("body").unbind("mousedown", app.zdsjyjmxgl.add.onBodyDown);
	},
	onBodyDown: function() {
		if (!(event.target.id == "menuBtn" || event.target.id == "menuContent" || $(event.target).parents("#" + app.zdsjyjmxgl.add
				.inputId + "Content").length > 0)) {
			app.zdsjyjmxgl.add.hideMenu();
		}
	},
	mxpzbdfz:function(){
		$.ajax({
