 ...
 * @Last Modified time: 2018-01-05 11:23:49
 */
app.qbxsfx = {
	 ...
	 * @return undefined
	 */
	initSslbTree: function(container, selectType = 1) {
		const treeContainer = $(container);
		
		let zTreeObj,
			zTreeOnClick = (event, treeId, treeNode) => {
				const sslbTreeDom = $(event.currentTarget);
				sslbTreeDom.siblings('.node-name').val(treeNode.qtlbmc);
				sslbTreeDom.siblings('.node-code').val(treeNode.sslb);
			},
			zTreeOnCheck = (event, treeId, treeNode) => {
				...
				for (let elem of nodes) {
					...
					codes.push(elem.sslb);
				}
				sslbTreeDom.siblings('.node-name').val(names.join(','));
				sslbTreeDom.siblings('.node-code').val(codes.join(','));
			},
			setting = {
				data: {
					simpleData: {
		...
		if (!app.constants.sslbTreeData) {
			app.alert('还没有请求到涉事类别数据！');
			return ;
		}
		zNodes = app.constants.sslbTreeData;
		if (selectType === 1) {
			zTreeObj = $.fn.zTree.init($(container + ' .sslb-tree-dom'), setting, zNodes);
		} else {
			zTreeObj = $.fn.zTree.init($(container + ' .sslb-tree-dom'), setting2, zNodes);
		}

		// 树选择输入框的点击 涉事类别
		treeContainer.find('.node-name').off('click').on('click', function(event) {
			const that = $(this);
			that.siblings('.sslb-tree-dom').css('display', 'block');
			const hideTreeDomFunc = (event) => {
				...
				if (!(target.hasClass('tree-dom') || target.parents('.tree-dom').length > 0)) {
					...
					that.siblings('.sslb-tree-dom').css('display', 'none');
				}
			};
			setTimeout(() => {
	 ...
	 * @return undefined
	 */
	initCjdwTree: function(container, selectType = 1) {
		const treeContainer = $(container);
		
		let zTreeObj,
			zTreeOnClick = (event, treeId, treeNode) => {
				const sslbTreeDom = $(event.currentTarget);
				sslbTreeDom.siblings('.node-name').val(treeNode.name);
				sslbTreeDom.siblings('.node-code').val(treeNode.code);
			},
			zTreeOnCheck = (event, treeId, treeNode) => {
				const sslbTreeDom = $(event.currentTarget),
					treeObj = $.fn.zTree.getZTreeObj(treeId),
		...
		if (!app.constants.cjdwTreeData) {
			app.alert('还没有请求到采集单位数据！');
			return ;
		}
		zNodes = app.constants.cjdwTreeData;
		if (selectType === 1) {
			zTreeObj = $.fn.zTree.init($(container + ' .cjdw-tree-dom'), setting, zNodes);
		} else {
			zTreeObj = $.fn.zTree.init($(container + ' .cjdw-tree-dom'), setting2, zNodes);
		}

		// 树选择输入框的点击
		treeContainer.find('.node-name').off('click').on('click', function(event) {
			const that = $(this);
			that.siblings('.cjdw-tree-dom').css('display', 'block');
			const hideTreeDomFunc = (event) => {
				const target = $(event.target);
				if (!(target.hasClass('tree-dom') || target.parents('.tree-dom').length > 0)) {
					$(document).off('click', hideTreeDomFunc);
					that.siblings('.cjdw-tree-dom').css('display', 'none');
				}
			};
			setTimeout(() => {
				$(document).on('click', hideTreeDomFunc);
	 ...
	 * @return undefined
	 */
	initPersonCategoryTree: function(container) {
		const treeContainer = $(container);
		...
		let zTreeObj,
			...
			zNodes = {};
		if (!app.constants.rylbTreeData) {
			app.alert('还没有请求到人员类别数据！');
			return ;
		}
		zNodes = app.constants.rylbTreeData;
