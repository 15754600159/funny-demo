define(function(require, exports, module) {
    ...
    function pagipationQuery(pageIndex,pageSize){
        // 查询数据
        //开始时间
        var kssj ="?kssj="+ $("#reservation").val();
       //行政区划代码
        var ssds_xzqhdm="&ssds_xzqhdm="+$("#area").val();
         //籍贯代码（省分）
        ...
        Post("/czrk/overview",{
            ...
            $.each(data.result, function (index, item) {
                    ...
                    htmlobj+="</td><td>"+(item.sfzh);
                    htmlobj+="</td><td>"+(item.xbdm==1?'男':'女');
                    htmlobj+="</td><td>"+(item.mz=="null"?'':item.mz)+"</td><td>" + (item.jg_sf==undefined?'':item.jg_sf);
                    htmlobj+= "</td><td>" + (item.nl==undefined?'':item.nl) + "</td><td>"+(item.zy=="null"?'':item.zy)+"</td><td>";
                    htmlobj+=(item.xlmc=="null"?'':item.xlmc)+"</td><td>"+(item.jlxmc=="null"?'':item.jlxmc)+"</td><td>"+(zdryxl[item.zdryxl]==undefined?'':zdryxl[item.zdryxl])+"</td></tr>";
            });
