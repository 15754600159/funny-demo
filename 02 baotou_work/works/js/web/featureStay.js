...
function ajaxNationBar(){
	...
	Get("/code/mzAll", function(data){
		...
		Get("/zzrk/features/mzdm", function(res){
...
function ajaxSex(){
	...
	Get("/zzrk/features/xbdm", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
			x.push(xb(k))
...
function ajaxNativePlace(){
	Get("/code/jgAll1", function(jgdm){
		sdCfg.jgdmSf = jgdm;
		Get("/zzrk/features/jg_sfdm", function(res){
					var x = [],y = [];
			$.each(res, function(k,v) {
				x.push(jg_sf(k))
				y.push(v)
...
ajaxAges();//初始化年龄
function ajaxAges(){
	Get("/zzrk/features/nl", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
...
function calendar(){
    ...
    $('#reservation').daterangepicker({
        ...
        console.log(startDate+''+endDate);//起始时间和结束时间
    });
...
function presentAddressBar(x,y){//现住址
	var myPresentAddress = echarts.init(document.getElementById('presentAddress'));
	var option = {
		    color: ['#3398DB'],
		    tooltip : {
		        trigger: 'axis',
	...
	myPresentAddress.setOption(option);
	window.addEventListener("resize", function () {
		myPresentAddress.resize()
	});
}
});