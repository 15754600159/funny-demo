...
function ajaxNationBar(){
	...
	Get("/code/mzAll", function(data){
		...
		Get("/personLqyd/features/mzdm", function(res){
...
function ajaxSex(){
	...
	Get("/personLqyd/features/xbdm", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
			x.push(xb(k))
...
function ajaxNativePlace(){
	Get("/code/jgAll1", function(jgdm){
		sdCfg.jgdmSf = jgdm;
		Get("/personLqyd/features/jgdm_sf", function(res){
			var x = [],y = [];
			$.each(res, function(k,v) {
				console.log(k)
				x.push(jg_sf(k))
...
ajaxAges();//初始化年龄
function ajaxAges(){
	Get("/personLqyd/features/nl", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
...
function ajaxJobs(){
	Get("/code/zylbAll", function(zylbdm){
		sdCfg.zylb = zylbdm;
		Get("/personLqyd/features/zylbdm", function(rs){
			var map = {},x = [],y=[];
...
function ajaxEducation(){
	Get("/code/whcdAll", function(data){
		sdCfg.whcd = data;
		Get("/personLqyd/features/xldm", function(rs){
			var map = {}, x = [],y=[];
			$.each(rs, function(k,v) {
				var xl = whcd(k);
...
function ajaxEmphasis(){
	Get("/code/zdryxlAll", function(zdryxldm){
		...
		Get("/personLqyd/features/zdryxl", function(rs){
			var map = {},x = [],y=[];
			$.each(rs, function(k,v) {
