...
function queryClickBind(){
    ...
    $('#query').off('click').on('click', function(){
        ...
        var data = {
            ...
            endTime: dataArray[2],
            hbh: $('#myDataHbh option:selected').text(),
            cfd_dwbh: $('#myDataChuFaDi option:selected').text(),
            mdd_dwbh: $('#myDataMuDiDi option:selected').text(),
            ...
            success: function(person){
                ...
                for(var i = 0; i<(person.total + 1); i++){
                    ...
                    htmlCode += `<tr class="page${page}">
                                    <td>${personGroup[i].hbh?personGroup[i].hbh:'-'}</td>
                                    ...
                                    <td>${personGroup[i].cfd_dwbh?personGroup[i].cfd_dwbh:'-'}</td>
                                    <td>${personGroup[i].mdd_dwbh?personGroup[i].mdd_dwbh:'-'}</td>
                                    <td>${personGroup[i].lgsj?personGroup[i].lgsj:'-'}</td>
                                    <td>${personGroup[i].ddsj?personGroup[i].ddsj:'-'}</td>
