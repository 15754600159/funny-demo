...
gulp.task('release:copy', function(cb) {
	gulp.src(['favicon.ico', 'fonts/**', 'umeditor/**', 'images/**', 'pages/**', 'index.html', 'css/*.min*.css', 'js/*.min*.js'], {
		cwd: paths.dist.root,
		base: paths.dist.root
	}).pipe(gulp.dest(paths.release.root + 'dlpmc-web/')).on('end', cb);
});

gulp.task('release:compress', function(cb) {
	gulp.src(paths.release.root + 'dlpmc-web/**').pipe(zip('dlpmc-web.zip')).pipe(gulp.dest(paths.release.root)).on(
		'end', cb);
});
