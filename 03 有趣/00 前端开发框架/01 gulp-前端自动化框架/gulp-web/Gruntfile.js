...
module.exports = function(grunt) {
	...
	for (let item in modules) {
		...
		concat['js'].files.push({
			src: modules[item].jsFiles,
			dest: paths.dist.js + item + '.js'
		});
		uglify['js'].files.push({
			src: paths.dist.js + item + '.js',
			dest: paths.dist.js + item + '.min-' + modules[item].version + '.js'
		});
		...
		cssmin['css'].files.push({
			...
			dest: paths.dist.css + item + '.min-' + modules[item].version + '.css'
		});
	}
	...
	grunt.initConfig({
		...
		copy: {
			...
			release: {
				files: [{
					expand: true,
					cwd: paths.dist.root,
					src: ['favicon.ico', 'fonts/**', 'images/**', 'pages/**', 'index.html', 'css/*.min*.css',
						'js/*.min*.js'],
					dest: paths.release.root + 'dlpmc-web/'
				}]
			}
		},
		compress: {
			release: {
				options: {
					archive: paths.release.root + 'dlpmc-web.zip',
					mode: 'zip'
				},
				files: [{
					expand: true,
					cwd: paths.release.root + 'dlpmc-web/',
					src: ['**']
				}]
			}
		},
		...
		watch: {
			src: {
				files: [paths.source.root + '**', 'modules.json'],
				tasks: ['dist-dev'],
				options: {
					livereload: 4001
				}
