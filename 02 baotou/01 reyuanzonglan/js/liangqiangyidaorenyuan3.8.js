...
function queryClickBind(){
    ...
    $('#query').off('click').on('click', function(){
        ...
        var data = {
            ...
            ajlbdm: $('#myDataCase option:selected').text(),
            from: pages,
            size1: size
        }
            ...
            success: function(person){
                ...
                for(var i = 0; i<(person.total + 1); i++){
                    ...
                    htmlCode += `<tr class="page${page}">
                                    ...
                                    <td>${personGroup[i].gmsfhm?personGroup[i].gmsfhm:'-'}</td>
                                    <td>${personGroup[i].xbdm?personGroup[i].xbdm:'-'}</td>
                                    <td>${personGroup[i].mzmc?personGroup[i].mzmc:'-'}</td>
                                    ...
                                    <td>${personGroup[i].zatdmc?personGroup[i].zatdmc:'-'}</td>
                                    <td>${personGroup[i].zy?personGroup[i].zy:'-'}</td>
                                    <td>${personGroup[i].xlmc?personGroup[i].xlmc:'-'}</td>
                                    <td>${personGroup[i].xzz_jlxmc?personGroup[i].xzz_jlxmc:'-'}</td>
                                    <td>${personGroup[i].zdrylbmc?personGroup[i].zdrylbmc:'-'}</td>
                                </tr>`;
                }
