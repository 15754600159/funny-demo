...
function ajaxNationBar(){
	...
	Get("/code/mzAll", function(data){
		...
		Get("/czrk/features/mzdm", function(res){
...
function ajaxSex(){
	...
	Get("/czrk/features/xbdm", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
			x.push(xb(k))
...
function ajaxNativePlace(){
	Get("/code/jgAll1", function(jgdm){
		sdCfg.jgdmSf = jgdm;
		Get("/czrk/features/jg_sfdm", function(res){
				var x = [],y = [];
			$.each(res, function(k,v) {
				x.push(jg_sf(k))
				console.log(k+'--'+jgdm[k].code)
				y.push(v)
			});
			nativePlaceBar(x,y);
		});
...
ajaxAges();//初始化年龄
function ajaxAges(){
	Get("/czrk/features/nl", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
...
function ajaxJobs(){
	Get("/code/zylbAll", function(zylbdm){
		sdCfg.zylb = zylbdm;
		Get("/czrk/features/zylbdm", function(rs){
			var map = {},x = [],y=[];
...
function ajaxEducation(){
	Get("/code/whcdAll", function(data){
		sdCfg.whcd = data;
		Get("/czrk/features/xldm", function(rs){
			var map = {}, x = [],y=[];
			$.each(rs, function(k,v) {
				var xl = whcd(k);
...
function ajaxAccent(){
	Get("/code/kyAll", function(hykydm){
		sdCfg.hyky = hykydm;
		Get("/czrk/features/hykydm", function(rs){
			var map = {},x = [],y=[];
