...
function ajaxNationBar(){
	...
	Get("/code/mzAll", function(data){
		...
		Get("/shedu/features/mzdm", function(res){
...
function ajaxSex(){
	...
	Get("/shedu/features/xbdm", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
			x.push(xb(k))
...
function ajaxNativePlace(){
	Get("/code/jgAll1", function(jgdm){
		sdCfg.jgdmSf = jgdm;
		Get("/shedu/features/jgdm_sf", function(res){
				var x = [],y = [];
			$.each(res, function(k,v) {
				x.push(jg_sf(k))
				y.push(v)
...
ajaxAges();//初始化年龄
function ajaxAges(){
	Get("/shedu/features/nl", function(res){
		var x = [],y = [],map = {};
		$.each(res, function(k,v) {
...
function ajaxJobs(){
	...
	Get("/code/zylbAll", function(zylbdm){
		sdCfg.zylb = zylbdm;
		Get("/shedu/features/zylbdm", function(rs){
			var map = {},x = [],y=[];
...
function ajaxEducation(){
	Get("/code/whcdAll", function(data){
		sdCfg.whcd = data;
		Get("/shedu/features/xldm", function(rs){
			var map = {}, x = [],y=[];
			$.each(rs, function(k,v) {
				var xl = whcd(k);
...
function ajaxAccent(){
	Get("/code/kyAll", function(hykydm){
		sdCfg.hyky = hykydm;
		Get("/shedu/features/hykydm", function(rs){
			var map = {},x = [],y=[];
...
function apartmentBar(data,name){//居住地
...
//      			axisTick: {show: false},
		    ...
		    yAxis : [
		        {
		            type : 'category',
		            //data : ['口音', '口音', '口音', '口音', '口音', '口音', '口音'],//name
					data:x,
//		            axisLine: {show: false},
//			        axisTick: {show: false},
//			        axisLabel: {interval: 0},
...
function specialityBar(x,y){//专长
	...
	var option = {
		    ...
		    xAxis : [
		        {
		            type : 'category',
		            //data : ['专长', '专长', '专长', '专长', '专长', '专长', '专长','专长','专长','专长'],//name
					data:x,
		            axisTick: {
		                alignWithLabel: true
		            }
